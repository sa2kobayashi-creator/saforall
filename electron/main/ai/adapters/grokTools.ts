...
export type GrokToolsCallParams = {
  secret: string
  baseUrl: string
  model: string
  extraHeaders?: string[]
...
/**
 * Grok (xAI) Chat Completions tool calling. Same contract as openaiTools; providerId=grok.
 */
export async function grokChatCompletionsWithTools(
  params: GrokToolsCallParams
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    try {
      ...
      try {
        ...
        throw aiErrorFromLlmHttp('grok', response.status, bodyText)
      }

      if (!response.ok) {
        const err = aiErrorFromLlmHttp('grok', response.status, bodyText)
          ...
          if (includeTools && /tool_choice|tools|function/i.test(lower)) {
            if (/not support|unsupported|does not support/i.test(lower)) {
              throw aiErrorFromCaught(
                'grok',
                new Error(
                  `${err.message} — この Grok モデルは function calling 未対応の可能性があります。設定で grok-4-7 など対応モデルを選んでください。`
                )
              )
            }
          }
      ...
      if (!/LLM HTTP 400/i.test(lastError.message) || attempt >= 3) {
        throw aiErrorFromCaught('grok', lastError)
      }
  ...
  throw aiErrorFromCaught('grok', lastError ?? new Error('LLM request failed'))
}
