...
/** Official xAI Chat Completions. Default model from docs.x.ai (Grok 4.6). */
const DEFAULT_MODELS = ['grok-4.6']
const DEFAULT_BASE_URL = 'https://api.x.ai/v1'

function buildGrokAskBody(
  model: string,
...
export const grokAdapter: AIProviderAdapter = {
  ...
  availableModels() {
    return parseSettingModels(getLocalSetting('llm.grok.models', ''), [
      getLocalSetting('llm.grok.model', DEFAULT_MODELS[0]),
      ...DEFAULT_MODELS
    ])
  },

  async generate(request: AIRequest, credential: Credential): Promise<AIResponse> {
    const model = request.model || getLocalSetting('llm.grok.model', DEFAULT_MODELS[0])
    const baseUrl = (
      request.baseUrl ||
      credential.baseUrl ||
      getLocalSetting('llm.grok.base_url', DEFAULT_BASE_URL) ||
      DEFAULT_BASE_URL
    ).replace(/\/$/, '')
    ...
    try {
      response = await fetch(url, {
        ...
        body: JSON.stringify(buildGrokAskBody(model, request.messages, request.temperature))
      })
    } catch (error) {
      const message = error instanceof Error ? error.message : 'network error'
      throw new AIError('NETWORK_ERROR', message, { providerId: 'grok' })
    }
    const text = await response.text()
    if (!response.ok) {
      throw aiErrorFromHttp('grok', response.status, text)
    }
    let json: Parameters<typeof extractContent>[0]
    try {
      json = JSON.parse(text) as Parameters<typeof extractContent>[0]
    } catch {
      throw new AIError('PROVIDER_ERROR', 'Grok の応答が JSON ではありません', {
        providerId: 'grok',
    ...
    if (!parsed.content.trim()) {
      throw new AIError('PROVIDER_ERROR', 'Grok から本文を取得できませんでした', {
        providerId: 'grok'
      })
    }
  ...
  async generateWithTools(request, credential, options) {
    ...
    const completion = await (await import('./grokTools')).grokChatCompletionsWithTools(callParams)
